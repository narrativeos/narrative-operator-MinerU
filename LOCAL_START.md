...
```bash
docker build -f docker/china/Dockerfile -t registry.cn-beijing.aliyuncs.com/futuremeng/mineru:latest .
```


push 镜像：
```bash
docker push registry.cn-beijing.aliyuncs.com/futuremeng/mineru:latest
```