...
async def local_parse_file(
    ...
    返回:
        成功: {"status": "success", "content": "Markdown内容"}
        失败: {"status": "error", "error": "错误信息"}
    """
...
async def parse_documents(
    ...
    # 通用参数
    enable_ocr: Annotated[bool, Field(description="启用OCR识别，默认False。当parse_method为auto时，此参数决定是否使用ocr方法")] = False,
    language: Annotated[
    ...
    page_ranges: Annotated[
        str | None,
        Field(
            description='指定页码范围，格式为逗号分隔的字符串。例如："2,4-6"：表示选取第2页、第4页至第6页；"2--2"：表示从第2页一直选取到倒数第二页。（仅远程API），默认None'
        ),
    ] = None,
    # 本地API参数（USE_LOCAL_API=true时有效）
    backend: Annotated[
        str,
        Field(
            description='解析后端，仅本地API有效。可选: "pipeline"、"vlm-auto-engine"、"vlm-http-client"、"hybrid-auto-engine"（默认）、"hybrid-http-client"'
        ),
    ] = "hybrid-auto-engine",
    parse_method: Annotated[
        str,
        Field(
            description='解析方法，仅本地API有效（pipeline/hybrid后端）。"auto"（默认）、"txt"、"ocr"。显式指定时优先于enable_ocr参数'
        ),
    ] = "auto",
    formula_enable: Annotated[
        bool, Field(description="是否启用公式解析，仅本地API有效，默认True")
    ] = True,
    table_enable: Annotated[
        bool, Field(description="是否启用表格解析，仅本地API有效，默认True")
    ] = True,
    server_url: Annotated[
        Optional[str],
        Field(
            description="OpenAI兼容服务器地址，仅本地API的<vlm/hybrid>-http-client后端有效，例如 http://127.0.0.1:30000"
        ),
    ] = None,
    start_page_id: Annotated[
        int, Field(description="起始页码（从0开始），仅本地API有效，默认0")
    ] = 0,
    end_page_id: Annotated[
        int, Field(description="结束页码（从0开始），仅本地API有效，默认99999")
    ] = 99999,
    enable_cache: Annotated[
        Optional[bool],
        Field(description="是否启用解析缓存，仅本地API有效，默认None（使用服务器配置）"),
    ] = None,
) -> Dict[str, Any]:
    """
    ...
    if config.USE_LOCAL_API:
        ...
        for path in file_paths:
            try:
                ...
                # 确定 parse_method：显式指定优先，否则根据 enable_ocr 推断
                effective_parse_method = parse_method
                if parse_method == "auto" and enable_ocr:
                    effective_parse_method = "ocr"
...
async def _parse_file_local(
    ...
    Args:
        ...
        language: 文档语言，用于OCR识别准确性
        backend: 解析后端
        formula_enable: 是否启用公式解析
        table_enable: 是否启用表格解析
        server_url: OpenAI兼容服务器地址（仅http-client后端有效）
    ...
    # 准备用于上传文件的表单数据
    # FastAPI List[UploadFile] 用同名字段 "files" 接收多个文件
    MIME_MAP = {
        ".pdf": "application/pdf",
        ".png": "image/png",
        ".jpg": "image/jpeg",
    ...
    file_type = MIME_MAP.get(file_path_obj.suffix.lower(), "application/octet-stream")
    ...
    if enable_cache is not None:
        form_data.add_field("enable_cache", str(enable_cache).lower())

    config.logger.debug(f"发送本地API请求到: {api_url}")
    config.logger.debug(
        f"上传文件: {file_path_obj.name} (大小: {len(file_data)} 字节), "
        f"backend={backend}, parse_method={parse_method}, language={language}"
    )
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(api_url, data=form_data) as response:
                ...
                config.logger.debug(
                    f"本地API响应 (keys): {list(result.keys()) if isinstance(result, dict) else type(result)}"
                )

                # 处理响应: {"backend":..., "version":..., "results": {"<stem>": {"md_content": "..."}}}
                if "error" in result:
                    return {"status": "error", "error": result["error"]}

                pdf_stem = file_path_obj.stem
                file_result = result.get("results", {}).get(pdf_stem, {})
                md_content = file_result.get("md_content")
                if md_content is None:
                    # fallback: 尝试第一个key
                    first = next(iter(result.get("results", {}).values()), {})
                    md_content = first.get("md_content", "")

                return {"status": "success", "content": md_content, "raw": result}
